...
测试包含：36 名角色数据完整性、两轮蛇形选秀、四局完整自动模拟、秘密选角网络视图。项目也已在四个独立浏览器标签页中验证房间加入与选秀同步，并在 390×844 手机视口检查布局。
...
本仓库不包含规则书 PDF 原图或卡牌插画。当前角色采用原创几何徽章和程序化 3D 棋子，方便后续替换为已获得数字发行授权的正式素材。
