...
function racerCard(id:string,enabled:boolean,action:string){const r=RACER_BY_ID[id];return`<button class="racer-card" style="--racer:${r.color}" data-card="${enabled?action:''}" ${enabled?'':'disabled'}><span class="racer-art">${r.icon}</span><b>${r.nameZh}</b><small>${r.name}</small><p>${r.powerZh}</p></button>`;}
